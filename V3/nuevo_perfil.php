<?php
    ...
    if (isset($_SESSION['usuario'])) {
        ...
        $consulta_profesionista = mysqli_query($conexion, "SELECT * FROM portafolio WHERE id_profesionista='$id_usuario'");
        $consulta_reclutador = mysqli_query($conexion, "SELECT * FROM reclutador WHERE id_reclutador='$id_usuario'");
...
<head>
    ...
    <title>Nuevo perfil | CVenLinea </title>
...
<main>
    <?php
    if(mysqli_num_rows($consulta_profesionista) > 0){
        ...
        $portafolio = mysqli_fetch_assoc($consulta_profesionista);
        //$_SESSION['id_portafolio'] = $portafolio['id_portafolio'];
        
    }else if(mysqli_num_rows($consulta_reclutador) > 0){
        //Existe en la tabla reclutador, 
        /*$reclutador = mysqli_fetch_assoc($consulta_reclutador);
        $_SESSION['id_reclutador'] = $reclutador['id_reclutador'];*/
    }else{ echo '<section>
                <h2>¿Eres un profesionista o un reclutador?</h2>
                <p></p>
                <a href="login.php" class="btn btn-secondary">Reclutador</a>
                <a href="profesionista/crear_cv.php" class="btn btn-secondary">Profesionista</a>
                </section>';
    }
}
    ...
    ?>
    <section>
        <h2>Ultimos CV Subidos</h2>
        <p>Texto</p>
     <div class="gallery">
        <div class="gallery-individual-container">
            <pre><img src="tulio2.jpg" alt="tulio" width="250" height="250">  <img src="juan.jpg" alt="juan" width="250" height="250">  <img src="poli.jpg" alt="poli" width="250" height="250"></pre>
            <span style="margin-right: 10px;">Texto con margen a la derecha</span>   <span style="margin-right: 60px;">Texto con margen a la derecha</span>   <span style="margin-right: 10px;">Texto con margen a la derecha</span>
            <pre></pre>
<div style="display: flex; gap: 150px;">
    <div><a href="#">Ver mas</a></div>
    <div><a href="#">Ver mas</a></div>
    <div><a href="#">Ver mas</a></div>
</div> 
</div>
